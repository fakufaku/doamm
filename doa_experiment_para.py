...
eval_list=[(n_sources,reverb,"./doa_result_{}_{}.txt".format(n_sources,reverb["name"]),"./doa_raw_{}_{}.txt".format(n_sources,reverb["name"])) for n_sources in n_sources_list for reverb in reverb_list]
#評価する手法群
...
mic_pairs_dict={}
mic_pairs_dict["pairs_1"]=[[m1,m2] for m1 in range(n_channels-1) for m2 in range(m1+1,np.minimum(m1+1+1,n_channels)) ]
mic_pairs_dict["pairs_2"]=[[m1,m2] for m1 in range(n_channels-1) for m2 in range(m1+1,np.minimum(m1+2+1,n_channels)) ]
mic_pairs_dict["pairs_4"]=[[m1,m2] for m1 in range(n_channels-1) for m2 in range(m1+1,np.minimum(m1+4+1,n_channels)) ]

#手法リスト
#grid_list=[180*90,90*90,90*45,60*30,30*15,18*9,12*6]
grid_list=[30*15,18*9,12*6]
algo_meta_info={}

for grid in grid_list:
    algo_meta_info["MDSBF_{}".format(grid)]={"algo_name":"MDSBF","mic_pairs":mic_pairs_dict["pairs_1"],
                                      "n_mm_iterations":5,"n_bisec_search":8,"n_rough_grid":12*6,"n_precise_grid":grid}
    algo_meta_info["SRP_{}".format(grid)]={"algo_name":"SRP","mic_pairs":mic_pairs_dict["pairs_1"],
                                      "n_mm_iterations":5,"n_bisec_search":8,"n_rough_grid":12*6,"n_precise_grid":grid}
    algo_meta_info["MUSIC_{}".format(grid)]={"algo_name":"MUSIC","mic_pairs":mic_pairs_dict["pairs_1"],
                                      "n_mm_iterations":5,"n_bisec_search":8,"n_rough_grid":12*6,"n_precise_grid":grid}
#提案法のリスト
...
#n_mm_iterations_list=[1,2,3,5,10]
n_mm_iterations_list=[1,2]
#mic_array_list=["pairs_1","pairs_2","pairs_4"]
mic_array_list=["pairs_1"]
#for rough_grid in rough_grid_list:
#    for n_mm_iterations in n_mm_iterations_list:
#        for mic_array in mic_array_list:
#            algo_meta_info["SPIRE_MM_{}_{}_{}".format(rough_grid,n_mm_iterations,mic_array)]={"algo_name":"SPIRE_MM","mic_pairs":mic_pairs_dict[mic_array],
#                                          "n_mm_iterations":n_mm_iterations,"n_bisec_search":8,"n_rough_grid":rough_grid,"n_precise_grid":180*90}



#rmse: n_sample, n_source
def eval_func(rmse,use_sample_mean=False,percentiles=[5,10,25,50,75,90,95], admit_errors=[0.5,1.,2.,3.,4.,5.,10.,20.]):
    ...
    out_dict={"mean":eval_mean,"median":eval_median,"std":eval_std,"eval_percentile":eval_percentile,"eval_accuracy":eval_accuracy}
...
def doa_experiment(algo_meta_info,eval_cond,R,seed=0):
    ...
    for rep in range(n_repeat_actual):
        ...
        for meta_info_name in algo_meta_info:
            ...
            doa = pra.doa.algorithms[algo_name](R, fs, stft_nfft, dim=3, c=c, n_grid=meta_info["n_precise_grid"],mic_positions=R.T,n_mm_iterations=meta_info["n_mm_iterations"],n_bisec_search=meta_info["n_bisec_search"],n_rough_grid=meta_info["n_rough_grid"],mic_pairs=meta_info["mic_pairs"])
        ...
        for meta_info_name in algo_meta_info:
            ...
            elapsed_time=np.average(elapsed_time)
            rmse=np.array(error_rmse)
            f.write("{} wave {} [sec] elapsed {} [sec]\n".format(meta_info_name,wave_time,elapsed_time))
        ...
        admit_errors=[0.5,1.,2.,3.,4.,5.,10.,20.]
        ...
        for meta_info_name in algo_meta_info:
            ...
            out_dict=eval_func(rmse,False,percentiles=percentiles,admit_errors=admit_errors)
            f.write("{},{},{},{},".format(meta_info_name,out_dict["mean"],out_dict["median"],out_dict["std"] ))
