...
class SPIRE_MM(DOA):
    ...
    def _make_rough_mode_vectors(self,n_rough_grid=None):
        ...
        if not isinstance(self, FRIDA):
            self.rough_mode_vec = ModeVector(self.L, self.fs, self.nfft, self.c, self.rough_grid)
    ...
    def _process_old2(self, X):
        ...
        d=np.array(self.mic_positions[mic_pairs[:,1]])-np.array(self.mic_positions[mic_pairs[:,0]])
        #d: n_mic_pair,dim
        print(np.shape(d))
        ...
        a=gsv.obtain_steering_vector(self.mic_positions,3.*source_position,self.freq_hz,useAmp=False)
        #10,55,48
        print(np.shape(a))
        ...
        for k  in self.freq_bins:
            ...
            sigma=np.where(np.abs(sigma)<1.e-18,np.zeros_like(sigma)+1.e-18,sigma)
            ...
            source_locations = geom.spherical_to_cartesian(
                doa=doas, distance=distance
            )

            position_vector[k,:,:]=source_locations[:self.dim,:].T
            
            size=np.einsum("ti,ti->t",np.conjugate(position_vector[k,:,:]),position_vector[k,:,:])
            size=np.sqrt(size)[...,np.newaxis]
            position_vector[k,:,:]=position_vector[k,:,:]/np.maximum(size,1.e-18)
            ...
            for i in range(self.n_mm_itertaions):
                #
                org_cost_0,org_cost_1,org_cost_2,org_cost_3,cost_0,cost_1,cost_2,cost_3,est_p,z,x= doa_estimation_one_iteration(d_temp,est_p,sigma,z,x,freqs,use_clustering=use_clustering,cluster_index=cluster_index,cluster_center=cluster_center,iter_num2=self.n_bisec_search)
        ...
        size=np.sqrt(size)[...,np.newaxis]
        position_vector=position_vector/np.maximum(size,1.e-18)
        ...
        grid_locations = geom.spherical_to_cartesian(
                doa=doas, distance=distance
        )
        size=np.einsum("in,in->n",np.conjugate(grid_locations),grid_locations)
        size=np.sqrt(size)[np.newaxis,...]
        grid_locations=grid_locations/np.maximum(size,1.e-18)
    ...
    def _process_debug(self, X):
        ...
        #print(np.concatenate([self.rough_grid.x[0:10,None],self.rough_grid.y[0:10,None],self.rough_grid.z[0:10,None]],axis=1))
        rough_grid_temp=np.concatenate([self.rough_grid.x[0:10,None],self.rough_grid.y[0:10,None],self.rough_grid.z[0:10,None]],axis=1)

        
        temp_mode=ModeVector2(self.mic_positions.T,16000,256,343,self.grid,mode="far")

    

        #source_positionとrough_gridのx,y,zは等しい
        source_position_org=[[0.42440754,0.43631269,2.93760267],
                         [ 1.35210912,-0.83474138,-2.54460365]]
        source_position=source_position_org
        #一番近い，vectorを見つけて，そのsteeringvectorを可視化してみる。
        grid_temp=np.concatenate([self.grid.x[:,None],self.grid.y[:,None],self.grid.z[:,None]],axis=1)
        
        size=np.einsum("im,im->i",np.conjugate(grid_temp),grid_temp)
        size=np.sqrt(size)[...,np.newaxis]
        grid_temp=grid_temp/np.maximum(size,1.e-18)
        
        size=np.einsum("im,im->i",np.conjugate(source_position),source_position)
        size=np.sqrt(size)[...,np.newaxis]
        source_position=source_position/np.maximum(size,1.e-18)
        ...
        a=gsv.obtain_steering_vector(self.mic_positions,300.*np.array(source_position_org),self.freq_hz,useAmp=False,SOUND_SPEED=343)
       ...
       # print(np.shape(mode_vec))
        ...
        position_vector[self.freq_bins,:,:]=np.transpose(source_locations[:self.dim,:,:],(1,2,0))
        ...
        size=np.sqrt(size)[...,np.newaxis]
        cluster_center=cluster_center/np.maximum(size,1.e-18)
    ...
    def _process_org(self, X):
        ...
        for i in range(self.n_mm_itertaions):
            #
            org_cost_0,org_cost_1,org_cost_2,org_cost_3,cost_0,cost_1,cost_2,cost_3,est_p,z,x= doa_estimation_one_iteration(d,est_p,sigma,z,x,freqs,use_clustering=use_clustering,cluster_index=cluster_index,cluster_center=cluster_center,iter_num2=self.n_bisec_search)
            #print(cost_1-cost_0,cost_2-cost_1,cost_3-cost_2)
            #print(org_cost_0,org_cost_3,org_cost_3-org_cost_0)
            print(cost_0,cost_1,cost_2,cost_3)
        #est_pから
...
class SPIRE_MM_CIRCULAR(DOA):
    ...
    def _process(self, X):
        ...
        for i in range(self.n_mm_itertaions):
            #
            #org_cost_0,org_cost_1,org_cost_2,org_cost_3,cost_0,cost_1,cost_2,cost_3,est_p,z,x= doa_estimation_one_iteration(freqs_d,est_p,sigma,z,x,use_clustering=use_clustering,cluster_index=cluster_index,cluster_center=cluster_center,iter_num2=self.n_bisec_search,silent_mode=silent_mode)
            org_cost_0,org_cost_1,org_cost_2,org_cost_3,cost_0,cost_1,cost_2,cost_3,est_p,z,x,x_non_const_power=coplaner_doa_estimation_one_iteration(freqs_d,est_p,sigma,z,x,use_clustering=use_clustering,cluster_index=cluster_index,cluster_center=cluster_center,iter_num2=self.n_bisec_search,silent_mode=silent_mode,zero_feature_index=2)
...
def coplanar_least_squares_st_norm_one(a,r,alpha,zero_feature_index=0,iter_num=10,eps=1.e-18):
    ...
    #左端を求める
    left_side=-1.*np.min(np.real(W),axis=-1)
    #右端を求める
    right_side=np.max(coef_p_abs-np.real(W),axis=-1)

    inv_W=1./np.maximum(np.real(W),eps)
    ...
    def cost(lamb):
       value=np.sum(p_2/np.maximum(np.square(lamb[...,np.newaxis]+np.real(W)),eps),axis=-1)-1.
       return(value)
    ...
    #print(lamb)
    #print(np.shape(lamb))
    inv_lamb_W=1./np.maximum(lamb[...,np.newaxis]+np.real(W),eps)
    ...
    y_result=np.sqrt(np.maximum(1.-x_power,eps))
    ...
    if pre==0:
        ...
        x_result=np.concatenate((x_result[...,:zero_feature_index],y_result[...,None],x_result[...,zero_feature_index:]),axis=-1)
...
def linear_least_squares_st_norm_one(a,r,alpha,feature_index=0,eps=1.e-18):
    ...
    cov=np.einsum("fti,fi,fi->ft",alpha,a[...,feature_index],np.conjugate(a[...,feature_index]))
    ...
    y_result=np.sqrt(np.maximum(1.-x_result*x_result,eps))
    if feature_index==0:
        temp_x=np.concatenate((x_result[...,np.newaxis],y_result[...,np.newaxis]),axis=-1)
    else:
        temp_x=np.concatenate((y_result[...,np.newaxis],x_result[...,np.newaxis]),axis=-1)
...
#r: freq,time,sample
#alpha: freq,time, sample
def least_squares_st_norm_one(a,r,alpha,iter_num=10,eps=1.e-18):
...
#補助関数の値
def calc_auxiliary_function_cost(freqs_d,p,sigma,z,x,SOUND_SPEED=343.,eps=1.e-8):
    #補助変数を更新する
    #tau=-np.einsum("pd,ftd->ftp",d,p)/SOUND_SPEED
    two_pi_f_tau=-np.einsum("fpd,ftd->ftp",freqs_d,p)*(2.*np.pi/SOUND_SPEED)
    
    #temp=2.*np.pi*np.einsum("f,ftp->ftp",freqs,tau)+sigma+2.*np.pi*z
    temp=two_pi_f_tau+sigma+2.*np.pi*z
    org_cost=np.cos(temp)
    sign_x=np.sign(x)
    sign_x=np.where(np.abs(sign_x)<0.3,np.ones_like(sign_x),sign_x)
    x_eps=np.maximum(np.abs(x),eps)*sign_x
    
    alpha=np.where(np.abs(x)<eps,-0.5*np.ones_like(x),-0.5*np.sin(x_eps)/x_eps)
...
def linear_doa_estimation_one_iteration(freqs_d,non_zero_feature_index,p,sigma,z,x,use_clustering=False,cluster_index=None,cluster_center=None,SOUND_SPEED=343.,silent_mode=False,eps=1.e-18):
    ...
    #print(x[400,0,:])
    if silent_mode==False:
        org_cost_0,cost_0=calc_auxiliary_function_cost(freqs_d,p,sigma,z,x,SOUND_SPEED,eps)
    else:
        org_cost_0=0
        cost_0=0
    ...
    #補助変数を更新する
    #tau=-np.einsum("pd,ftd->ftp",d,p)/SOUND_SPEED
    two_pi_tau=-np.einsum("fpd,ftd->ftp",freqs_d,p)*(2.*np.pi/SOUND_SPEED)
    
    coef=two_pi_tau+sigma
    
    right_side=0.5-coef/(2.*np.pi)
    #zを変更
    z=np.floor(right_side)
    
    
    if silent_mode==False:
        org_cost_1,cost_1=calc_auxiliary_function_cost(freqs_d,p,sigma,z,x,SOUND_SPEED,eps)
    else:
        org_cost_1=0
        cost_1=0
    ...
    if silent_mode==False:
        org_cost_2,cost_2=calc_auxiliary_function_cost(freqs_d,p,sigma,z,x,SOUND_SPEED,eps)
    else:
        org_cost_2=0
        cost_2=0
    ...
    alpha=np.where(np.abs(x)<1.e-8,np.ones_like(x),np.sin(x_eps)/x_eps)
    ...
    if use_clustering==True:
        ...
        two_pi_f_cluster_tau=-np.einsum("fpd,cd->fcp",freqs_d,cluster_center)*(2.*np.pi/SOUND_SPEED)

        #cluster_tau=-np.einsum("pd,cd->cp",d,cluster_center)/SOUND_SPEED
        cluster_x=two_pi_f_cluster_tau[:,np.newaxis,:,:]+sigma[...,np.newaxis,:]+2.*np.pi*z[...,np.newaxis,:]
        ...
        for c in range(cluster_num):
            ...
            #
            p_temp=linear_least_squares_st_norm_one(a3[np.newaxis,...],r3[np.newaxis,np.newaxis,...],alpha3[np.newaxis,np.newaxis,...],feature_index=non_zero_feature_index,eps=eps)
        ...
        p=linear_least_squares_st_norm_one(a,r,alpha,feature_index=non_zero_feature_index,eps=eps)
    ...
    if silent_mode==False:
        org_cost_3,cost_3=calc_auxiliary_function_cost(freqs_d,p,sigma,z,x,SOUND_SPEED,eps)
    else:
        org_cost_3=0
        cost_3=0
    return(org_cost_0,org_cost_1,org_cost_2,org_cost_3,cost_0,cost_1,cost_2,cost_3,p,z,x)
...
#SOUND_SPEED: 343
def doa_estimation_one_iteration(freqs_d,p,sigma,z,x,use_clustering=False,cluster_index=None,cluster_center=None,iter_num2=100,SOUND_SPEED=343.,silent_mode=False,eps=1.e-18):
    freq_num=np.shape(p)[0]
    frame_num=np.shape(p)[1]
    pair_num=np.shape(freqs_d)[1]
    feature_dim=np.shape(freqs_d)[2]
...
#SOUND_SPEED: 343
def coplaner_doa_estimation_one_iteration(freqs_d,p,sigma,z,x,use_clustering=False,cluster_index=None,cluster_center=None,iter_num2=100,SOUND_SPEED=343.,silent_mode=False,zero_feature_index=2,eps=1.e-18):
    freq_num=np.shape(p)[0]
    frame_num=np.shape(p)[1]
    pair_num=np.shape(freqs_d)[1]
    feature_dim=np.shape(freqs_d)[2]
    ...
    if use_clustering==True:
                ...
                for l in range(frame_num):
                    if cluster_index[k,l]==c:
                        if a3 is None:
                            ...
                            alpha3=np.concatenate((alpha3,alpha2[k,l,:]),axis=0)
            #
            lamb_temp,p_temp,x_non_const_power=coplanar_least_squares_st_norm_one(a3[np.newaxis,...],r3[np.newaxis,np.newaxis,...],alpha3[np.newaxis,np.newaxis,...],zero_feature_index=zero_feature_index,iter_num=iter_num2,eps=eps)
            #p_temp: 1, feature_dim
            cluster_center[c,:]=p_temp
        #pを作る
        for k in range(freq_num):
        ...
        lamb,p,x_non_const_power=coplanar_least_squares_st_norm_one(a,r,alpha,zero_feature_index=zero_feature_index,iter_num=iter_num2,eps=eps)
    #confirm_solutions(a,r,alpha,lamb)
    ...
    if silent_mode==False:
        ...
        org_cost_3=0
        cost_3=0
    return(org_cost_0,org_cost_1,org_cost_2,org_cost_3,cost_0,cost_1,cost_2,cost_3,p,z,x,x_non_const_power)
